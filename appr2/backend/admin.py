...
def send_revision_email(signer_name: str, signer_email: str, token: str, building: Optional[str], notes: str):
    """Send an email notifying the applicant that revisions are needed."""
    signing_url = f"{BASE_URL}/pub_apply/{token}"
    building_text = f" for <strong>{building}</strong>" if building else ""
    ...
    <head>
        ...
        <style>
            body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f8fafc; margin: 0; padding: 0; }}
            .container {{ max-width: 560px; margin: 40px auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08); }}
            .header {{ background: linear-gradient(135deg, #b45309 0%, #d97706 100%); padding: 36px 40px; }}
            .header h1 {{ color: white; margin: 0; font-size: 22px; font-weight: 700; letter-spacing: -0.3px; }}
            .body {{ padding: 36px 40px; }}
            .body p {{ color: #374151; font-size: 15px; line-height: 1.6; margin: 0 0 16px; }}
            .cta-button {{ display: inline-block; background: #b45309; color: white !important; text-decoration: none; padding: 14px 32px; border-radius: 10px; font-weight: 600; font-size: 15px; margin: 8px 0 24px; }}
            .info-box {{ background: #fffbeb; border-left: 4px solid #f59e0b; padding: 16px 20px; margin: 20px 0; }}
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Action Required: Application Revision</h1>
            </div>
            <div class="body">
                <p>Hi <strong>{signer_name}</strong>,</p>
                <p>The leasing team has reviewed your rental application{building_text} and requested some updates before it can be approved.</p>
                <div class="info-box">
                    <p><strong>Message from the team:</strong><br/>{notes}</p>
                </div>
                <p>Please click the button below to securely update your information and re-sign the documents.</p>
                <a href="{signing_url}" class="cta-button">Update My Application →</a>
            </div>
        </div>
    </body>
    </html>
    """
    text = f"Hi {signer_name},\n\nRevisions are requested for your application.\nNotes: {notes}\n\nUpdate here: {signing_url}"
    return send_email_smtp(to_recipients=[signer_email], subject="Action Required: Application Revision", html_body=html, text_body=text)
...
# --- PUT /{id}/request-revision — Unlock app & notify user ---
@router.put("/{application_id}/request-revision")
def request_revision(application_id: int, payload: ApplicationRevision, db: Session = Depends(get_db)):
    app = db.query(Application).filter(Application.id == application_id).first()
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")

    app.status = "Revision Requested"
    app.revision_notes = payload.revision_notes
    
    # Clear the generated signed PDFs so they must be regenerated
    app.signed_form_410_id = None
    app.signed_consent_id = None
    ...
    if package:
        package.status = "sent"
        sessions = db.query(SigningSession).filter(SigningSession.package_id == package.id).all()
        for session in sessions:
            session.status = "in_progress"
            session.signatures = {}  # Clear signatures
    ...
    db.commit()
    logger.info(f"🔄 Application {application_id} set to Revision Requested. Notes: {payload.revision_notes}")

    return {
        "status": "success",
        "message": "Application unlocked and signers notified.",
    }
