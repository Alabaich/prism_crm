...
def notify_admin_late_upload(signer_name: str, lead_name: str, filename: str):
    """Sends an email to the admin when a document is uploaded to a completed application."""
    admin_email = os.getenv("ADMIN_EMAIL", "customercare@prismpm.ca")
    subject = f"Late Document Upload: {lead_name}"
    html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        <style>
            body {{ font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f4f4f7; padding: 20px; }}
            .container {{ max-width: 500px; margin: 0 auto; background: #ffffff; border-radius: 8px; padding: 20px; border: 1px solid #e2e8f0; }}
            h3 {{ color: #0f172a; margin-top: 0; }}
            p {{ color: #334155; line-height: 1.5; }}
            .highlight {{ background: #f8fafc; padding: 10px; border-radius: 6px; border-left: 4px solid #3b82f6; margin: 15px 0; font-family: monospace; }}
        </style>
    </head>
    <body>
        <div class="container">
            <h3>New Document Added to Completed Application</h3>
            <p><strong>{signer_name}</strong> has just uploaded an additional document to an application that was already marked as completed.</p>
            <div class="highlight">{filename}</div>
            <p>Please check the admin dashboard to review this new document.</p>
        </div>
    </body>
    </html>
    """
    text = f"New document uploaded by {signer_name}: {filename}. Please check the dashboard."
    
    # Fire and forget
    try:
        send_email_smtp(to_recipients=[admin_email], subject=subject, html_body=html, text_body=text)
    except Exception as e:
        logger.error(f"Failed to send late upload notification: {e}")
...
async def upload_document(
    ...
    if app:
        ...
        elif category == "additional_doc":
            # Safely append to JSON list
            existing_docs = app.additional_doc_ids or []
            new_list = list(existing_docs)
            new_list.append(doc.id)
            app.additional_doc_ids = new_list
            from sqlalchemy.orm.attributes import flag_modified
            flag_modified(app, "additional_doc_ids")
    ...
    # If the session was already completed, this is a late addition. Notify the admin.
    if session.status == "completed":
        lead = db.query(Lead).filter(Lead.id == package.lead_id).first()
        lead_name = lead.prospect_name if lead and lead.prospect_name else session.signer_name
        notify_admin_late_upload(session.signer_name, lead_name, file.filename or safe_name)
